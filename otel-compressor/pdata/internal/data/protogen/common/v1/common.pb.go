...
func (m *AnyValue) Marshal() (dAtA []byte, err error) {
	  ...
	  if m_, ok := m.Value.(*AnyValue_KvlistValue); ok {
	    ret_ := make(map[string][]interface{})
	    ret_["kvlist_value"] = make([]interface{}, 0)
	    for _, Kv := range m_.KvlistValue.Values {
	      d_, err :=json.Marshal(Kv.Value.Value)

	      if err != nil {
	        return nil, err
	      }
	      d__ := make(map[string]interface{})
	      err = json.Unmarshal(d_, &d__)
	      if err != nil {
	        return nil, err
	      }
	      ret_["kvlist_value"] = append(ret_["kvlist_value"], map[string]interface{}{
	        "key": Kv.Key,
	        "value": d__,
	      })
	    }
